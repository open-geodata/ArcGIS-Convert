    ...
    try:
        if pretty:
            ...
            ET.ElementTree(gpx).write(
                gpxFile, encoding="UTF-8", xml_declaration=True)
...
def generatePointsFromFeatures(inputFC, descInput, zerodate=False):
    ...
    def attHelper(row):
        ...
        valuesDict["NAME"] = row[fieldNameDict["NAME"]
                                 ] if "NAME" in fields else " "
        valuesDict["DESCRIPT"] = row[fieldNameDict["DESCRIPT"]
                                     ] if "DESCRIPT" in fields else " "
        ...
        if "DATETIMES" in fields:
            ...
            formatted_time = time.strftime(
                "%Y-%m-%dT%H:%M:%SZ", time.gmtime(0)) if zerodate else " "
                ...
                if descInput.shapeType == "Polyline":
                    for part in row:
                        try:
                            ...
                            arcpy.AddWarning(
                                "Problem reading values for row: {}. Skipping.".format(row[0]))
                    ...
                    trkType = row[fieldNameDict["TYPE"]
                                  ].upper() if "TYPE" in fields else None
                    try:
                        attHelper(row)
    ...
    valuesDict = {"ELEVATION": 0, "NAME": "", "DESCRIPT": "",
                  "DATETIMES": "", "TYPE": "", "PNTX": 0, "PNTY": 0}
    fieldNameDict = {"ELEVATION": 0, "NAME": 1, "DESCRIPT": 2,
                     "DATETIMES": 3, "TYPE": 4, "PNTX": 5, "PNTY": 6}
    ...
    for index, gpxValues in enumerate(getValuesFromFC(inputFC, cursorFields)):
        ...
        if gpxValues[0] == "wpt":
            wpt = ET.SubElement(
                gpx, 'wpt', {'lon': valuesDict["PNTX"], 'lat': valuesDict["PNTY"]})
            ...
            trkPt = ET.SubElement(
                trkSeg, "trkpt", {'lon': valuesDict["PNTX"], 'lat': valuesDict["PNTY"]})
